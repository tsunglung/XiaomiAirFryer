    ...
    if config_entry.options[CONF_FLOW_TYPE] == CONF_DEVICE:
        ...
        async def async_service_handler(service):
            ...
            if entity_ids:
                devices = [
                    device
                    for device in hass.data[DATA_KEY][DATA_DEVICE].values()
                    if device.entity_id in entity_ids
                ]
            else:
                devices = hass.data[DATA_KEY][DATA_DEVICE].values()
...
class XiaomiAirFryer(SwitchEntity):
    ...
    async def async_food_quanty(self, food_quanty: int):
        """Set recipe id."""
        self._device.food_quanty(recipe_id)
